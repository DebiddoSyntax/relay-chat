...
export function useChatSocket(activeId: number | null, shouldScrollRef: React.RefObject<boolean>) {
    ...
    useEffect(() => {
        ...
        const connect = () => {
            ...
            const handleMessage = async (event: MessageEvent) => {
                ...
                if (data.type === "chat.message.partial") {
                    setAiTyping(false);
                    setPartialText((prev) => prev + data.content);
                }
                    ...
                    setMessages(prev => {
                        ...
                        if (activeId) {
                            messagesCache[activeId] = { 
                                messages: updated, 
                                nextUrl: messagesCache[activeId]?.nextUrl ?? null 
                            };
                        }
                        // console.log('socket cache', messagesCache[activeId].nextUrl)
                        return updated;
                    });
    ...
    return { sendMessage, aiTyping, status, partialText, socketRef }
